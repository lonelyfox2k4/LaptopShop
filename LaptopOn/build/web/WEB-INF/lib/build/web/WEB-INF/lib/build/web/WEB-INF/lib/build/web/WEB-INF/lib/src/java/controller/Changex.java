 ...
 * @author admin
 */
@WebServlet(name="Changex", urlPatterns={"/changex"})
public class Changex extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Changex</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Changex at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        model.User u = d.getUserByEmail(user);
        if (!u.getPass().equals(pass)) {
            request.setAttribute("error", "Mật khẩu sai!!!");
            request.getRequestDispatcher("changePasswordAdmin.jsp").forward(request, response);
        } else {
            d.changePassword(user, passchange);
